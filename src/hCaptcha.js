...
(async () => {
  ...
  // console.log(settings.APIKEY, "APIKEY");
  ...
  (async () => {
    ...
    async function solve() {
      ...
      if (settings.debugModeGridOnly == "true" && !isGrid()) {
        document.querySelector(".button-submit").click();
        return;
      }
      ...
      if (Object.keys(images).length === 0 || target === "") return;

      const searchParams = new URLSearchParams(location.hash);
      const type = isMulti() ? "multi" : isBbox() ? "bbox" : "grid";
      log(type, target, cells, images, examples, choices);
      if (logs) console.log(type, target, cells, images, examples, choices);
    ...
    async function sliceOG() {
      ...
      // Create a new canvas context optimized for frequent readbacks
      ...
      readbackCtx.drawImage(originalCanvas, 0, 0);

      // Now use this optimized context for getImageData
      const originalImageData = readbackCtx.getImageData(
        0,
        0,
        originalWidth,
        originalHeight
      ...
      if (allPixelsTransparentOrBlack) {
        console.log("The original canvas has no valid content");
        return null;
      }
      ...
      if (desiredWidth <= 0 || desiredHeight <= 0) {
        console.log("Desired width and height should be positive numbers");
        return null;
      }
      ...
      // console.log(originalCanvas.toDataURL("image/jpeg"));
    ...
    async function handleMulti(images, examples) {
      if (!(examples && images)) return;
      const bg = document.querySelector(".task-image .image").style.background;
      if (!bg) return;
      let singleImg = "";
      try {
        singleImg =
    ...
    async function handleBbox(images, examples) {
      if (!(images && examples)) return;
      ...
      await Promise.all(
        Array.from(document.querySelectorAll(".example-image .image")).map(
          async (e) => {
            const url = e.style.background.match(/url\("(.+?)"\)/)?.[1];
            if (url !== null && url !== undefined) {
              examples.push(await getBase64FromUrl(url));
            }
          }
        )
      );

      // let example_images = document.querySelectorAll(".example-image .image");
      // for (const element of example_images) {
      //   let urlMatch = element.style.background.match(/url\("([^"]+)"\)/);
      //   if (urlMatch) {
      //     try {
      //       const base64 = await getBase64FromUrl(urlMatch[1]);
      //       examples.push(base64);
      //     } catch (error) {
      ...
      // }
      // log("images", images);
      // console.log(examples, "examplesexamples");
    }
        ...
        const check_interval = setInterval(async function () {
          ...
          if (isGrid()) {
            ...
            examples = await Promise.all(
              Array.from(
                document.querySelectorAll(".challenge-example .image .image")
              ).map(async (e) => {
                const url = e.style.background.match(/url\("(.+?)"\)/)?.[1];
                if (url !== null && url !== undefined) {
            ...
            await handleMulti(images, examples);
          } else if (isBbox()) {
            await handleBbox(images, examples);
          }
