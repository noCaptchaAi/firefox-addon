...
(async () => {
  ...
  class TimeUtil { static now() { return Date.now() } static currentDate() { return new Date() } static sleep(t) { return new Promise((e) => setTimeout(e, t)) } static async randomSleep(t, e) { const n = Math.floor(Math.random() * (e - t) + t); return await Time.sleep(n) } }
  ...
  const jsNotif = (m, d) => { const t = document.createElement("div"); t.style.cssText = "position:fixed;top:10%;left:0;background-color:rgba(0,0,0,.8);border-radius:4px;padding:16px;color:#fff;font:calc(14px + .5vw) 'Arial',sans-serif;font-weight:bold;text-transform:uppercase;letter-spacing:1px;z-index:9999;transition:all 1s;animation:slideIn 1s forwards", t.innerHTML = m, document.body.appendChild(t); const o = document.createElement("style"); o.innerHTML = "@keyframes slideIn{0%{transform:translateX(-100%)}100%{transform:translateX(0)}}@keyframes slideOut{0%{transform:translateX(0)}100%{transform:translateX(100%)}}", document.head.appendChild(o); setTimeout(() => { t.style.animation = "slideOut 1s forwards", setTimeout(() => { document.body.removeChild(t) }, 1e3) }, d || 3e3) };
  const isRecaptchaVisible = () => document.querySelector(".recaptcha-checkbox") !== null;
  const isWidgetFrame = isRecaptchaVisible;

  const isImageFrame = () => document.querySelector(".rc-imageselect-instructions") !== null;
  ...
  const isSolved = () => {
    ...
    const isImageFrameSolved = document.querySelector("#recaptcha-verify-button")?.disabled;
    return isWidgetFrameSolved || isImageFrameSolved;
  };

  const hasSolveError = () =>
    document.querySelector(".rc-doscaptcha-header")?.innerText === "Try again later";
  ...
  async function handleWidgetFrame() {
    ...
    if (settings.RautoOpen) fireMouseEvents(document.querySelector("#recaptcha-anchor"));
  }
  ...
  async function handleSpeechFrame() {
    if (settings.RautoSolve === false || isSolved() || hasSolveError()) return;

    const downloadUrl =
      document.querySelector(".rc-audiochallenge-tdownload-link")?.href ||
      document.querySelector("#audio-source")?.src;

    let language = document.querySelector("html")?.getAttribute("lang")?.trim() || "en";

    const solveStart = TimeUtil.now();
    const audioapi = "https://audio.nocaptchaai.com/solve";
    const [text, id] = await getAudioResponse(audioapi, settings.apikey, downloadUrl);
  ...
  async function checkFrameVisibility(selector) {
    const frames = [
      ...document.querySelectorAll(`iframe[src*="/recaptcha/api2/${selector}"]`),
      ...document.querySelectorAll(`iframe[src*="/recaptcha/enterprise/${selector}"]`)
    ];

    return frames.some(frame => window.getComputedStyle(frame).visibility === "visible");
  }
