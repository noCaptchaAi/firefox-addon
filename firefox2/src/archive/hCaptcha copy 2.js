(async () => {
    ...
    async function solve() {
        ...
        try {
            ...
            console.log("answer", answer, "status", status, "error", error);
            ...
            if (error) {
                ...
                if (isMulti()) {
                    ...
                    console.log("resprgtonse", response);
                    const indices =
                        status === STATUS.NEW
                            ? response.solution
        ...
        function getRemainingTime(status, elapsedTime) {
            if (isMulti()) {
                return SLEEP_TIME_MULTI - elapsedTime;
            } else if (isBbox()) {
                return SLEEP_TIME_BBOX - elapsedTime;
            } else {
                return SLEEP_TIME_DEFAULT - elapsedTime;
            }
        }
        ...
        async function handleFetchError() {
            try {
                ...
                if (fetchResponse.statusText === "Forbidden") {
                    jsNotif(
                        "⚠ Server blocked this IP, please wait 10 minutes for unblocked."
                    );
                    console.log("Error", fetchResponse.statusText);
                    sleep(IP_BLOCKED_SLEEP_TIME);
                    return;
                }
            } catch (error) {
                console.log(error);
